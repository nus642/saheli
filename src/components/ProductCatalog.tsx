...
export function getProductDetails(product: Product, variantId?: string): ProductDetails {
  ...
  return product.variants.find((variant) => variant.id === variantId) ?? getDefaultVariant(product)!;
}
    ...
    <section className="bg-[#FAF9F6] py-12 lg:py-20" id="products">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div className="text-center max-w-3xl mx-auto space-y-3 font-sans">
          <div className="inline-block px-3 py-1 border border-[#5B6346] text-[#5B6346] text-[10px] uppercase tracking-widest font-semibold">Verified Product Catalog</div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 font-sans">
          {categories.map((category) => <button key={category.id} onClick={() => setSelectedCategory(category.id)} className={`px-5 py-2.5 text-xs sm:text-sm uppercase tracking-wider font-semibold transition-all cursor-pointer ${selectedCategory === category.id ? 'bg-[#5B6346] text-white' : 'bg-white text-[#2C2C2C] border border-[#E5E2D9] hover:border-[#5B6346]'}`}>{category.label}</button>)}
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-sans">
          {filteredProducts.map((product) => {
            const details = getProductDetails(product);
            return <article key={product.id} className="bg-white border border-[#E5E2D9] overflow-hidden shadow-xs hover:border-[#5B6346] transition-all flex flex-col group">
              <div className="relative h-80 bg-[#E8E2D6] overflow-hidden"><img src={details.imagePaths[0]} alt={`${product.name}正面包装`} className="w-full h-full object-contain group-hover:scale-[1.02] transition-transform duration-500" /></div>
              <div className="p-6 flex-1 flex flex-col gap-4">
                ...
                <div className="flex flex-wrap gap-1.5">{product.tags.map((tag) => <span key={tag} className="bg-[#F2F0E9] text-[#2C2C2C] border border-[#D8D4C7] text-[10px] font-medium px-2 py-0.5">{tag}</span>)}</div>
                <div className="pt-3 border-t border-[#E5E2D9] flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => openDetails(product)} className="px-3 py-2 border border-[#5B6346] text-[#5B6346] text-xs font-semibold flex items-center gap-1 hover:bg-[#F2F0E9] transition-colors cursor-pointer"><Eye className="w-3.5 h-3.5" />查看详情</button>
                    <a href={details.purchaseUrl} target="_blank" rel="noopener noreferrer" className="px-3 py-2 bg-[#5B6346] hover:bg-[#4A5039] text-white text-xs font-semibold flex items-center gap-1 transition-colors">前往淘宝查看<ExternalLink className="w-3.5 h-3.5" /></a>
                  </div>
                </div>
              </div>
            </article>;
        ...
        {activeProduct && activeDetails && <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="product-detail-title">
          <div className="bg-[#FAF9F6] border border-[#E5E2D9] max-w-3xl w-full p-6 sm:p-8 space-y-6 relative shadow-xl max-h-[90vh] overflow-y-auto font-sans">
            ...
            <div className="flex items-start gap-4 border-b border-[#E5E2D9] pb-4 pr-10">
              <div className="flex gap-2 shrink-0">{activeDetails.imagePaths.map((imagePath, index) => <div key={imagePath} className="w-20 sm:w-24 h-28 bg-white overflow-hidden border border-[#E5E2D9]"><img src={imagePath} alt={`${activeProduct.name}${index === 0 ? '正面' : '背面'}包装`} className="w-full h-full object-contain" /></div>)}</div>
              <div>
            ...
            {activeProduct.kind === 'variant' && <fieldset className="space-y-2"><legend className="font-serif font-bold text-[#2C2C2C] text-sm">选择颜色</legend><div className="flex flex-wrap gap-2">{activeProduct.variants.map((variant) => <button key={variant.id} type="button" aria-pressed={variant.id === activeVariant?.id} onClick={() => setActiveVariantId(variant.id)} className={`px-4 py-2 border text-sm font-semibold cursor-pointer transition-colors ${variant.id === activeVariant?.id ? 'bg-[#5B6346] border-[#5B6346] text-white' : 'bg-white border-[#D8D4C7] text-[#2C2C2C] hover:border-[#5B6346]'}`}>{variant.colorName} <span className="font-normal">{variant.colorEnglishName}</span></button>)}</div></fieldset>}
            ...
            <a href={activeDetails.purchaseUrl} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto px-5 py-3 bg-[#5B6346] hover:bg-[#4A5039] text-white text-sm font-semibold flex items-center justify-center gap-2 transition-colors">前往淘宝查看{activeVariant && `（${activeVariant.colorName}）`}<ExternalLink className="w-4 h-4" /></a>
          </div>
        </div>}
      </div>
    </section>
