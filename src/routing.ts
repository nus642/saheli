...
export const DEFAULT_PAGE_METADATA = {
  ...
  path: '/',
  imagePath: '/assets/brand/rajasthan-botanical-region-concept.webp',
} as const;
...
export const KNOWLEDGE_PAGE_METADATA = {
  ...
  path: KNOWLEDGE_PATH,
  imagePath: '/assets/guides/saheli-two-step-dye-guide.jpg',
} as const;

export function getInitialTab(pathname: string): string {
  return pathname.replace(/\/+$/, '') === KNOWLEDGE_PATH.replace(/\/+$/, '') ? 'knowledge' : 'home';
}
